...
try:
	...
	from .utils import *
from upload.models import RTROI, RTContour, ROI
from upload.models import RTStructureSet
from AlgoEngine.app.AlgoEngine.RadiationTherapyDecisionSupport.Python.General.extractROINames import preprocess_roi_name_ucla
...
def parse(dicom_dataframe,user,patient,study,series):
	...
	for roi in roi_sequence:
		try:
			...
			rt_roi = RTROI()
			roi_name = [x.ROIName for x in roi_label_sequence if
							  x.ROINumber == roi.ReferencedROINumber][0]
			roi_name = preprocess_roi_name_ucla(roi_name)
			try:
